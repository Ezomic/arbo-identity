...
defineOptions({ layout: { breadcrumbs: [{ title: 'Tenants', href: index().url }] } });
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4">
        <Heading title="New Tenant" description="Create a new tenant on this platform." />

        <form class="max-w-md space-y-4" @submit.prevent="submit">
            <div class="grid gap-2">
                <Label for="name">Organization name</Label>
                <Input id="name" v-model="form.name" @input="onNameInput" required autofocus placeholder="Acme Arbodienst" />
                <InputError :message="form.errors.name" />
            </div>

            <div class="grid gap-2">
                <Label for="slug">Slug</Label>
                <Input id="slug" v-model="form.slug" required placeholder="acme-arbodienst" />
